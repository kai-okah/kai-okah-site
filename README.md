...
One zustand store holds a mode state machine. Clicking a hotspot (an invisible box over a normal-looking object) is only legal in `idle`; it flies the camera along rails to a named pose, and when the flight lands, a plain-DOM overlay fades in over the canvas — 3D sells the zoom, DOM renders the text. `Esc` or "back" reverses it. A `PerformanceMonitor` steps a degradation ladder (shadows → resolution → particles) instead of letting weak GPUs stutter, and `prefers-reduced-motion` turns flights into cuts.
